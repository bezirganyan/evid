import argparse
from model.model_viz import Visualizer
from model.model import EpiModel
import numpy as np
...
parser.add_argument('--transmission_prob', help='The probability of transmitting the disease from infected person to a healthy one', type=float, required=True) 
parser.add_argument('--mortality_rate', help='The mortality rate of the disease', type=float, required=True)
parser.add_argument('--inf_radius', help='The radius within which the disease can be transmitted', type=float, required=True)
parser.add_argument('--healing_period', help='The amount of days after which the person is healed', type=int, required=True) 
parser.add_argument('--travel_prob', help='The probability of a person travelling to adel blocks', type=float, required=True)
parser.add_argument('--healthcare_potential', help='The portion of people from population, which the healthcare system can handle [0-1]', type=float, required=True)
